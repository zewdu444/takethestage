...
const TeachersTable: React.FC<TeachersTableProps> = ({ teachers, currentPage, totalPages, onPageChange, onRowClick, onDelete }) => {
    return (
        <>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {teachers.map((teacher) => (
                        <tr key={teacher.id} className="cursor-pointer">
                            <td className="px-6 py-4 whitespace-nowrap" onClick={() => onRowClick(teacher.id)}>{teacher.first_name} {teacher.second_name} {teacher.last_name}</td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <Button onClick={() => onDelete(teacher.id)}>Delete</Button> {/* Add Delete button */}
                            </td>
                        </tr>
                    ))}
                </tbody>
